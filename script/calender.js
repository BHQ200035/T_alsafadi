...
const dateSelectorView = document.getElementById('date-selector-view');
const timeSelectorView = document.getElementById('time-selector-view');
const detailsFormView = document.getElementById('details-form-view');
...
// **4. تحديث النصوص بناءً على الاختيار**
...
function updateTimeDisplay() {
    ...
    const datePart = document.getElementById('selected-date-display').textContent;
...
// **6. منطق التقويم الديناميكي والترتيب**
...
// 1. إضافة أسماء الأيام (رؤوس الجدول)
    ...
    // .getDay() يعطينا 0 للأحد، 1 للاثنين، إلخ.
    ...
    today.setHours(0, 0, 0, 0); // لضمان المقارنة الصحيحة لليوم
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        cell.textContent = day.toLocaleString('ar-EG'); // عرض الأرقام باللغة العربية
        cell.setAttribute('data-date', fullDate);
        
        // مقارنة التاريخ الحالي لتحديد ما إذا كان اليوم متاحاً أم لا
        if (currentDate < today) {
            cell.classList.add('disabled'); // يوم سابق
        } else {
...
function prevMonth() {
    ...
    selectedDate = null; // إزالة التحديد عند تغيير الشهر
    renderCalendar();
}
...
// **جديد: قائمة المناطق الزمنية (كمثال)**
const TIMEZONES_LIST = [
    ...
    // يمكنك إضافة المزيد من المناطق الزمنية هنا
];
...
const currentNameDisplay = document.getElementById('current-timezone-name');
const currentTimeDisplay = document.getElementById('current-timezone-time');
const searchInput = document.getElementById('timezone-search-input');


// **جديد: وظائف المنطقة الزمنية**
...
function openTimezoneDropdown() {
    ...
    searchInput.value = ''; // مسح خانة البحث عند الفتح
    searchInput.focus();
}
...
function selectTimezone(zone) {
    ...
    // 💡 ملاحظة: إذا كان تحديد المنطقة الزمنية يؤثر على الأوقات المتاحة، 
    // يجب استدعاء renderTimeSlots() هنا.
}


// **تحديث: استدعاء الوظيفة الجديدة في الإطلاق الأولي**
window.onload = () => {
    const today = new Date();
    ...
    renderCalendar();
    showDateView();
    // 🚀 جديد: عرض المنطقة الزمنية الافتراضية عند التحميل 🚀
    renderTimezoneDisplay();
};
